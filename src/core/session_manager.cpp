...
// -- 无线 Portal 认证线程（惰性启动，同 EAP/UDP） --
    ...
    // （上游 WebAuthProcess 路径：auto 后端 PortalWifi 使用；与 PortalProcess 并存）
                                     ...
                                     ConnectMode mode, const QStringList& allowedSsids)
{
    ...
    // 显式无线 Portal（本 PR authMode=wireless）：不依赖网卡/链路决策，直接认证
...
void SessionManager::stopConnection(bool logoutWifi, bool userInitiated)
{
    ...
    // （PortalProcess::stop 幂等：从未 start 时代数守卫 + wasOnline=false 不发请求）
...
// 显式无线（本 PR authMode=wireless）：Portal start() 自身幂等
    // （代数递增作废旧会话），直接重启
