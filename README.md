...
华南理工大学（SCUT）校园网认证客户端：**有线 802.1X（DrCOM）+ 无线 Web Portal（DrCOM eportal）**，支持 DrCOM 会话保持、系统托盘后台运行、断线自动重连、夜间断网自动恢复。无线侧提供**显式「无线 Portal」**（对接 `s.scut.edu.cn`）与**自动连接（有线优先）**两种接入方式。
...
- **无线 Portal 自动认证（显式模式）**：认证方式选「无线 Portal」后，直接对接 `s.scut.edu.cn`（DrCOM eportal），无需打开浏览器，程序自动完成状态查询 → 登录 → 在线保活全流程；掉线自动重新登录
- **自动连接（有线优先）**：网线插入→有线 802.1X；未插网线且当前 Wi-Fi 命中白名单→无线 Portal 自动识别门户、模拟浏览器登录、上线确认（`online_list` 官方接口轮询），校园网 0:00 强制登出后晨间自动再登（无线 SSID 白名单可配置，默认 `scut-student`，留空=任一 Wi-Fi）
...
- **连接**：顶部状态指示器 + 状态文案 + 当前连接模式；认证方式（有线 802.1X / 无线 Portal）；账号 / 密码（回车即连接）；主按钮「连接」与「断开」；底部深色运行日志（一键复制 / 清空）
...
1. **以管理员身份运行**程序
2. 填写校园网账号（学号）和密码；或在「设置」页将认证方式固定为「有线 802.1X」，选择对应的有线网卡，程序自动检测 IP/掩码/网关/MAC；如需静态 IP 勾选「连接时配置静态IP，断开时恢复DHCP」并填写网络参数
3. 点击**连接**

### 无线（scut-student 等 SSID）— 显式无线 Portal

1. 先在系统中连接校园网 Wi-Fi（如 `scut-student`）
2. 认证方式选择「无线 Portal」，填写校园网账号（学号）和密码，点击**连接**
...
认证方式为「自动」时无需手动选择：网线插入走有线 802.1X；未插网线且当前 Wi-Fi 命中
「无线网络名称」白名单（设置页，默认 `scut-student`，多个用逗号分隔，留空=任意）则走
无线 Portal。网络未就绪时程序静默等待，检测到有线接入或匹配 Wi-Fi 后自动发起认证。
被服务器夜间强制踢下线后自动等待到早上 6:00 重连（有线/无线一致）。
...
| 配置项 | 说明 |
|--------|------|
| 认证方式 | 自动（有线优先、无线兜底，默认）/ 有线 802.1X（宿舍网口）/ 无线 Portal（Wi-Fi） |
| 账号 / 密码 | 校园网统一认证账号和密码（有/无线通用） |
| 记住密码 | 密码经 Windows DPAPI 加密后保存在程序目录下的 `config.ini` |
| 无线网络名称 | SSID 白名单（逗号分隔，留空=任意；默认 `scut-student`；位于设置页） |
...
Portal URL 构造与 JSONP 解析 / 配置回环 / 无线门户解析与接入后端决策）有 QtTest 回归护栏，
无线侧覆盖 a79.htm 变量解析、online_list 查询构造、GBK 错误文案分类（密码错误→不可重试）、
会话归属校验、SSID 白名单；全量测试用例见 `tests/tst_packets.cpp`（夹具 `tests/wifi_fixtures.h`
为实测抓取的真实协议响应样本）：
...
├── ui/                   # UI：布局、系统托盘、网卡列表、认证方式切换、配置（mainwindow.cpp/h/ui）
├── core/                 # 连接编排与共享纯逻辑
│   ├── session_manager.cpp/h   # 状态机、线程管理、自动重连（有线/无线分支、后端决策）
...
│   └── log_level.h             # 日志级别常量
├── config/               # config.ini 读写（ConfigManager，含 ToAuthConfig）+
...
├── portal/               # 无线 Portal（显式模式，DrCOM eportal）
│   ├── portal_process.cpp/h        # 异步 HTTPS 认证状态机 + 在线保活
│   └── portal_protocol.cpp/h       # eportal 协议纯函数（URL 构造 / JSONP 解析 / 响应分类）
├── wifi/                 # 无线 Web Portal（自动模式，Dr.COM eportal）
